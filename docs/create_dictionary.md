...
You can find the code corresponding to the creation of a dictionary for the [Medical Subject Headings vocabulary](https://www.nlm.nih.gov/mesh/meshhome.html) in the [code folder](../code). The code at [parse_csv](../code/parse_csv.py) creates a dictionary of a CSV while [parser](../code/parser.py) takes a turtle TTL file as input. Please be aware that due to license restrictions, we can only provide the code but not the actual data for the corresponding dictionary.
