...
class CSVParser:
    """
    Class to parse the MESH CSV file, extract metadata for each MeSH vocabulary term and output a mwt dictionary.
    ...
    metadata : dict
        Dictionary containing the corresponding metadata for each term.
    """
        ...
        for i in range(len(data)):
            if data['Class ID'].iloc[i].startswith('http://purl.bioontology.org/ontology/MESH/'):
                ...
                if pd.isnull(data['Semantic Types'].iloc[i]) is False:
                    metadata[data['Class ID'].iloc[i]]['Semantic Types'] = \
                        data['Semantic Types'].iloc[i].replace("|", ", ")
        return metadata
